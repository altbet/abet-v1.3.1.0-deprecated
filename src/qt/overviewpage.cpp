...
public:
    ...
    inline void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        QRect amountRect(mainRect.left() + xspace + 4, mainRect.top() + ypad + 5, mainRect.width() - xspace - ICON_OFFSET, halfheight);
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeAltbetAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAltbetAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeAltbetAmount = strAnonymizeAltbetAmount.remove(strAnonymizeAltbetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAltbetAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeAltbetAmount * COIN) nMaxToAnonymize = nAnonymizeAltbetAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeAltbetAmount * COIN) {
        // ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                            //   .arg(strAnonymizeAltbetAmount));
        strAnonymizeAltbetAmount = strAnonymizeAltbetAmount.remove(strAnonymizeAltbetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAltbetAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
